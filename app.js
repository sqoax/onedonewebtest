...
function renderForecast() {
  ...
  if (state.forecastResults) {
    container.innerHTML = forecastResultsHTML(state.forecastResults);
    updateForecastTimer();
    return;
  }
  container.innerHTML = `
    <div class="card">
      <div class="card-header">
        ...
        <span class="status-pill neutral">10,000 sims</span>
      </div>
      <p class="help-text" style="margin-top:10px;">Run a probability model using current standings, observed weekly payouts, remaining purse estimates, and one-and-done depletion.</p>
      <button class="btn btn-primary" type="button" id="runForecastBtn" style="margin-top:16px;">Run Simulation</button>
    </div>
  `;
  $("runForecastBtn")?.addEventListener("click", runMonteCarlo);
}

function runMonteCarlo() {
  if (!state.sheetData.tournaments.length) {
    showToast("Sheet data is not loaded yet.", "error");
    return;
  }

  const button = $("runForecastBtn");
  if (button) {
    button.disabled = true;
  ...
  setTimeout(() => {
    const LEAGUE_LOG_MU = -4.687;
    const LEAGUE_LOG_SIGMA = 1.015;
    const LEAGUE_MISS_CUT_RATE = 0.167;
    const PRIOR_WEIGHT = 12;
    const DEGRADATION_RATE = 0.00000008;
    const HISTORICAL_PURSES = [9100000, 9200000, 9600000, 9600000, 20000000, 20000000];
    const TOURNAMENT_PURSES = [
      9600000, 20000000, 25000000, 9100000, 9900000, 9800000, 21000000,
      20000000, 20000000, 20000000, 19000000, 10300000, 9900000, 20000000,
      9800000, 21500000, 20000000, 8800000, 9000000, 17000000, 8800000,
      10000000, 8500000
    ];
    ...
    function sampleLogNormal(mu, sigma) {
      return Math.exp(mu + sigma * sampleNormal());
    }

    const weeksRemaining = Math.max(0, 23 - ((state.cfStatus.currentWeek || 1) - 1));
    const NUM_SIMS = 10000;
    const validTournaments = state.sheetData.tournaments.filter((t) => t.picks.some((p) => p.earnings > 0));
    const n = validTournaments.length;
    const memberStats = {};

    MEMBERS.forEach((member) => {
      if (n === 0) {
        memberStats[member] = {
          observedLogMu: LEAGUE_LOG_MU,
          observedMissCutRate: LEAGUE_MISS_CUT_RATE,
          totalEarnings: 0,
          n: 0
        };
        return;
      ...
      validTournaments.forEach((tournament, tournamentIndex) => {
        const pick = tournament.picks.find((p) => p.member === member);
        const earnings = pick ? pick.earnings || 0 : 0;
        totalEarnings += earnings;
        if (earnings === 0) {
          missCutCount++;
        } else {
          const histPurse = tournamentIndex < HISTORICAL_PURSES.length
            ? HISTORICAL_PURSES[tournamentIndex]
            : HISTORICAL_PURSES[HISTORICAL_PURSES.length - 1];
          madeWeeks.push(earnings / histPurse);
        }
      });

      memberStats[member] = {
        observedLogMu: madeWeeks.length
          ? madeWeeks.reduce((sum, value) => sum + Math.log(value), 0) / madeWeeks.length
          : LEAGUE_LOG_MU,
        observedMissCutRate: missCutCount / n,
        totalEarnings,
        n
      };
    });

    const leagueAvgTotal = 6 * (1 - LEAGUE_MISS_CUT_RATE) *
      Math.exp(LEAGUE_LOG_MU + 0.5 * LEAGUE_LOG_SIGMA ** 2) * 10000000;
    const memberProfiles = {};

    MEMBERS.forEach((member) => {
      const stats = memberStats[member];
      const shrinkage = PRIOR_WEIGHT / (PRIOR_WEIGHT + stats.n);
      const shrunkLogMu = shrinkage * LEAGUE_LOG_MU + (1 - shrinkage) * stats.observedLogMu;
      const shrunkMissCutRate = shrinkage * LEAGUE_MISS_CUT_RATE + (1 - shrinkage) * stats.observedMissCutRate;
      const outperformance = Math.max(0, stats.totalEarnings - leagueAvgTotal);
      const finalLogMu = Math.max(
        shrunkLogMu - outperformance * DEGRADATION_RATE,
        LEAGUE_LOG_MU - 1.5 * LEAGUE_LOG_SIGMA
      );
      const skillUncertainty = LEAGUE_LOG_SIGMA / Math.sqrt(stats.n + PRIOR_WEIGHT);
      memberProfiles[member] = { finalLogMu, shrunkMissCutRate, skillUncertainty };
    });

    const wins = {};
    MEMBERS.forEach((member) => { wins[member] = 0; });
    ...
    for (let sim = 0; sim < NUM_SIMS; sim++) {
      ...
      MEMBERS.forEach((member) => {
        simTotals[member] = state.sheetData.standings.totals?.[member] || 0;
        simLogMu[member] = memberProfiles[member].finalLogMu + sampleNormal() * memberProfiles[member].skillUncertainty;
      });

      for (let w = 0; w < weeksRemaining; w++) {
        const weekIndex = ((state.cfStatus.currentWeek || 1) - 1) + w;
        const futurePurse = weekIndex < TOURNAMENT_PURSES.length ? TOURNAMENT_PURSES[weekIndex] : 9500000;

        MEMBERS.forEach((member) => {
          if (Math.random() < memberProfiles[member].shrunkMissCutRate) return;
          let payoutPct = sampleLogNormal(simLogMu[member], LEAGUE_LOG_SIGMA);
          payoutPct = Math.min(payoutPct, 0.18);
          simTotals[member] += payoutPct * futurePurse;
        });
      }

      let bestMember = MEMBERS[0];
      let bestTotal = simTotals[MEMBERS[0]];
      for (let i = 1; i < MEMBERS.length; i++) {
        if (simTotals[MEMBERS[i]] > bestTotal) {
          bestTotal = simTotals[MEMBERS[i]];
    ...
    state.forecastResults = MEMBERS.map((member) => ({
      name: member,
      winPct: (wins[member] / NUM_SIMS) * 100
    })).sort((a, b) => b.winPct - a.winPct);
    state.forecastLastRun = Date.now();
    renderForecast();
    if (state.forecastTimer) clearInterval(state.forecastTimer);
    state.forecastTimer = setInterval(updateForecastTimer, 1000);
  }, 20);
}

function forecastResultsHTML(results) {
  const top = results[0]?.winPct || 1;
  return `
    <div class="card">
      <div class="card-header">
        <div>
        ...
        <span class="status-pill neutral" id="forecastTimer">Last run just now</span>
      </div>
      <div class="mini-list" style="margin-top:14px;">
        ${results.map((result, index) => `
          <div class="forecast-row">
            <span class="rank-num ${index === 0 ? "gold" : ""}">${index + 1}</span>
            <div class="row-main">
              <span class="row-title">${escapeHTML(result.name)}</span>
              <span class="progress-track"><span class="progress-fill ${index === 0 ? "gold" : ""}" style="width:${Math.max(2, (result.winPct / top) * 100)}%"></span></span>
            </div>
            <span class="percentage ${index === 0 ? "leader" : ""}">${result.winPct.toFixed(1)}%</span>
          </div>
        `).join("")}
      </div>
      <button class="btn btn-secondary" type="button" id="runForecastBtn" style="margin-top:16px;">Run Again</button>
    </div>
  `;
}

function updateForecastTimer() {
  const el = $("forecastTimer");
  if (!el || !state.forecastLastRun) return;
  const seconds = Math.floor((Date.now() - state.forecastLastRun) / 1000);
  if (seconds < 60) {
    el.textContent = `Last run ${seconds}s ago`;
  } else {
    el.textContent = `Last run ${Math.floor(seconds / 60)}m ago`;
  }
}
